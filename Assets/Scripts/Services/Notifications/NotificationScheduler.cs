...
namespace LocalCalendar.Notifications
{
    ...
    public static class NotificationScheduler
    {
        ...
        public static void SyncNotifications(CalendarItem oldItem, CalendarItem newItem)
        {
            bool hadReminder = oldItem != null &&
            oldItem.Type == CalendarItemType.Reminder &&
            oldItem.Reminder != null;

            bool hasReminder = newItem.Type == CalendarItemType.Reminder &&
            newItem.Reminder != null;

            // If there was an old reminder, we must cancel it using the old data/ID
            if (hadReminder)
            {
                NotificationScheduler.Cancel(oldItem); 
            }

            // Schedule the new one (Schedule already calls Cancel, but doing it 
            // above with oldItem ensures we catch the "stale" IDs)
            if (hasReminder)
            {
                Schedule(newItem);
            }
        ...
        public static void FireImmediate(CalendarItem item)
        {
            NotificationLogger.Log(new NotificationLogEntry
            {
                ...
                ScheduledLocal = DateTime.Now.AddSeconds(5),
                Note = "Immediately firing late notification"
            });

            // Construct a startUtc such that ComputeFireTimeUtc returns ~now + 5s:
            // fireTime = startUtc - offset - buffer
            // startUtc = now + 5s + offset + buffer
            DateTime fakeStartUtc = DateTime.UtcNow
